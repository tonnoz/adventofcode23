...
object Seven {
  ...
  private fun problemOne(input: List<String>){
    ...
    input
      .map { it.toHand() }
      .sorted()
      .foldIndexed(0){ i, acc , curr-> acc + curr.bidAmount * (i + 1) }
      .let { println(it) }
  ...
  private fun String.toHand(): Hand {
    val (cards, bidAmount) = this.split(" ")
    ...
    return Hand(categoryStrength, bidAmount.toShort(), cardsInt)
  }

  private class Hand(val categoryStrength: Int, val bidAmount: Short, val intValues : IntArray): Comparable<Hand> {
